import ./make-test.nix ({ lib, ... }:
{
  ...
  testScript = ''
    ...
    $machine->succeed(q(max_tests=10; sleep_time=2; for ((i=0; i<max_tests; i++)); do [[ $(btrfs fi du -s --raw /home/dedup-me-{1,2} | awk 'NR>1 { print $3 }' | grep -E '^0$' | wc -l) -eq 0 ]] && exit 0; sleep "$sleep_time"; done));
  '';
})
