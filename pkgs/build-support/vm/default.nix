...
rec {
  ...
  stage1Init = writeScript "vm-run-stage1" ''
    ...
    mkdir -p /fs${storeDir}
    mount -t 9p store /fs${storeDir} -o trans=virtio,version=9p2000.L,cache=loose,msize=$(( 1024 * 1024 * 4))
    ...
    mkdir -p /fs/tmp/xchg
    mount -t 9p xchg /fs/tmp/xchg -o trans=virtio,version=9p2000.L,msize=$(( 1024 * 1024 * 4))
